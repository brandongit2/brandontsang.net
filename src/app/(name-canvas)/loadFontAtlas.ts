...
export async function loadMsdfFontAtlas(): Promise<FontAtlas> {
	const fontText = await fs.readFile(path.resolve(__dirname, `../../../public/Karrik-Regular-msdf.json`), `utf-8`)
	const font = JSON.parse(fontText) as FontAtlas
	return font
}

export async function loadSdfFontAtlas(): Promise<FontAtlas> {
	const fontText = await fs.readFile(path.resolve(__dirname, `../../../public/Karrik-Regular-sdf.json`), `utf-8`)
	const font = JSON.parse(fontText) as FontAtlas
	return font
}
